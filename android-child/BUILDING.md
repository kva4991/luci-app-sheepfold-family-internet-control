...
Ответ имеет поля `ok`, `apiVersion`, `serverTime`, вложенный объект `data` и объект `error`. В `data` используются безопасные поля `deviceName`, `internetState`, `accessEndsAt`, `minutesRemaining`, `nextAccessChangeTime`, server-driven флаги функций и короткое `message` только для disabled/unknown. `accessMode`, конфликт и имя личной группы не передаются.
...
Release-задачи требуют внешний keystore и переменные из [`RELEASE_SIGNING.md`](RELEASE_SIGNING.md). Debug-сборка от production-ключа не зависит.
