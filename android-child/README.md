...
- периодический сетевой опрос и разовое восстановление после загрузки выполняются через WorkManager;
- `SafeBootReceiver` не удерживает сетевой запрос, а только ставит безопасную разовую задачу;
- точный AlarmManager остаётся только для уведомления о завершении доступа (§andwork1).
