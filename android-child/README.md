...
1. Приложение обращается к HTTPS API Sheepfold на едином порту приложения, по умолчанию `5201`.
2. При первом корректном ответе Sheepfold приложение закрепляет SHA-256-отпечаток сертификата локального роутера.
3. При следующих запросах изменение сертификата считается ошибкой безопасности.
