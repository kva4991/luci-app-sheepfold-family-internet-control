...
// Редактор владеет только локальным состоянием Wi-Fi-карточек. Запись UCI и
// перезапуск Wi-Fi переданы узкими callbacks, чтобы модуль можно было проверить
// отдельно от большого компоновщика страницы. §frontmod §wifitgl1
...
function create(deps) {
	...
	function register(editor) {
		editors.push(editor);
		[
			[editor.ssidInput, 'input'],
			[editor.passwordInput, 'input'],
			[editor.securitySelect, 'change'],
			[editor.channelSelect, 'change'],
			[editor.enabledInput, 'change']
	...
	function save() {
		...
		plan.items.forEach(function (item) {
			stageItem(item, plan.radiosToEnable);
		});

		// Снятие disabled с radio может включить все его AP. Поэтому интерфейсы,
		// которые родитель оставил выключенными, stageItem явно удерживает в OFF.
		Object.keys(plan.radiosToEnable).forEach(function (device) {
			deps.unsetOption(device, 'disabled');
		});

		return deps.save().then(deps.reload).then(function () {
			plan.items.forEach(function (item) {
