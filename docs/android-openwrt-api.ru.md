...
Рабочий локальный API сейчас доступен через `/cgi-bin/sheepfold-api`. Сопряжение родительского телефона использует короткоживущий одноразовый код, HTTPS с закреплением сертификата и Bearer-токен, привязанный к числовому ID и MAC админского устройства. Будущий alias `/api/v1/*` и refresh-token ещё не завершены, но текущий CGI pairing уже не является UI-заглушкой.
...
`true` означает, что на роутере установлен AI-IPK, ИИ включён, выбран поддерживаемый провайдер и для него сохранён ключ. Поле не раскрывает провайдера или ключ. При отсутствии поля Android использует `false`.
