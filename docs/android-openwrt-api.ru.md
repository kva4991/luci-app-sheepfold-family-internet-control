...
Этот документ описывает целевой контракт. В текущем пакете `0.1.0-79` уже есть локальный сервис на порту `5201`, discovery-файл `/.well-known/sheepfold.json` и CGI endpoint `/cgi-bin/sheepfold-api`, который отдаёт базовые сведения о роутере, версии, порте и пути API.
