...
Для защищённых текущих endpoint-ов backend отвечает `401` с машинным кодом: `device_unbound`, если заявленные ID/MAC больше не принадлежат админскому устройству, `invalid_token` для недействительного/истёкшего токена и `token_revoked`, если токен был отозван между проверками. Android по этим ответам начинает только новое сопряжение, не весь onboarding; `5xx` и транспортные ошибки credential не удаляют (§authrs1).
