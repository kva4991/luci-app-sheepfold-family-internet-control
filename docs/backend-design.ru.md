...
Endpoint для детского APK. Не требует токена администратора — идентифицирует устройство по IP из DHCP (§b5wkq2e).
...
- Журнал хранится в RAM (`/tmp/sheepfold/events.log`). При экспорте маскируются MAC-адреса и IP (§rtj5pht).
