...
`language`, `new_device_policy`, `detection_mode`, `update_check_install_mode`, `domain_allowlist_for_blocklist`, `site_*`, `site_filter_backend`, `adguard_auto_manage`, `adguard_health_interval_seconds`, AdGuard Home (`url`, `username`, `password`), `app_port`, messenger tokens, `integration_mode`, emergency sites, Wi-Fi auto, NTP/timezone, WPS, LED, `log_storage`, log retention, offline cleanup, USB, Yandex Disk (`login`, `password`, `root_folder`, `quota_mb`), AI keys/flags, `export_mode`, `blocked_page_text`, группы/устройства/списки MAC.
