...
| `app_https_port` | `global` | HTTPS-порт API (в UI только `app_port`) |
...
| Расписания (`schedule` секции) | Вкладка Schedules — **прототип**, кнопки не пишут UCI | |
| `bedtime` | Подпись «21:00» на экране расписаний | Не читает/не пишет `sheepfold.global.bedtime` |
