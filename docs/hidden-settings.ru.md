...
| `deepseek_model` default в defaults `deepseek-chat` vs UI `deepseek-v4-flash` | Расхождение default UCI и основного UI; отдельный старый `ai.js` снова использует `deepseek-chat` | До реализации ИИ модель и список допустимых значений нужно свести к одному источнику истины. |
...
## Устаревшие параметры без действующего runtime-смысла

| Параметр | Заметка |
| --- | --- |
| `allow_router_for_blocked` | Старый неоднозначный переключатель. Обычное ограничение интернета не закрывает LuCI само по себе; чёрный список устройств и identity-карантин закрывают управление роутером независимо от этого значения. |
| `block_router_for_blacklist` | Дублирует обязательное правило чёрного списка устройств и не читается runtime-кодом. Нужна миграция с удалением после проверки старых конфигов. |
