...
- версия OpenWrt-пакета в рабочей ветке — `0.1.0-r275`;
...
Прогресс ветки `chatGPT`: родительский APK получил рабочие редакторы расписаний и групп, безопасный экран администраторов, глобальное управление Wi-Fi и журнал. Общий `/api/v1/admin-config` не отдаёт пароли, pairing-коды и токены; изменения используют optimistic revision и UCI rollback. Локальная защита теперь снова блокирует APK через 1 минуту в фоне по умолчанию (доступны сразу/1/5/15 минут), после пяти ошибочных PIN/паролей ввод получает возрастающую задержку, Face/Fingerprint объединены в честную системную биометрию, а отключение интернета виджетом требует разблокировки и подтверждения, если владелец явно не включил предупреждённый instant-режим. Release-задачи fail-closed без внешних signing secrets. Создание администраторов/QR и сложные Wi-Fi-настройки остаются в LuCI. До завершения P2 остаются owner-controlled key, Android Lint/release-сборка, два физических телефона и live-router матрица.
