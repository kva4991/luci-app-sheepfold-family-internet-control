...
Локальное продолжение 09.09.2026: **r297**, база r296 `89b7093`. Исправлены B58–B60:
чужие аварийные каталоги не удаляются при совпадении PID, снимки проверяются до
мутации, Wi-Fi использует свою очередь UCI и отклоняет обнаруженные чужие черновики.
[Реестр](bug-register.ru.md) — 60 карточек; результаты — в [отчёте r297](audit-fixes-r297.ru.md).
R03/R10 остаются открыты: проверки очереди и ревизии не устраняют последнюю гонку
перед commit и не заменяют согласование всех writers. D01 сохранён.

Локальное продолжение 09.09.2026: **r296**, база r295 `5f2c816`. Исправлены B54–B57:
границы SSID/WPA-PSK, пароль для OWE и потеря аварийных копий при неудачном rollback.
[Реестр](bug-register.ru.md) — 57 карточек; результаты — в [отчёте r296](audit-fixes-r296.ru.md).
Следующий приоритет R03/R10: native UCI/pending-очередь, безопасное хранение аварийных
копий и подтверждение работы реального Wi-Fi. Полная транзакционность не объявлена. D01 сохранён.
Исторические контрольные точки ниже не заменяют текущую проверку.

Локальное продолжение 08.09.2026: **r294**, база доступного r293 `a1cb6cb`.
Внутренняя квота pairing/QR reset, отказоустойчивые ответы и обычный журнал исправлены
в B43–B46. Правки backup/staging альтернативного r293 отсутствовали в bundle и
повторно внесены как B47–B49. [Реестр](bug-register.ru.md) — 49 карточек;
[отчёт r294](audit-fixes-r294.ru.md). Профильно 178 тестов, без полного runner и оборудования.

Локальное продолжение 08.09.2026: **r293**, база r292. Исправлены B39–B42:
внешние HTTP-квоты, отказы хранения, обратный ход часов и ответы 429/503/Retry-After.
[Реестр](bug-register.ru.md) — 42 карточки, [отчёт](audit-fixes-r293.ru.md).
142 профильных теста в 8 файлах проходят; полный runner не запускался по решению владельца.
Следующий узкий объект — внутренний pairing-счётчик (R09); R03/R08 и живые проверки открыты.

Историческая контрольная точка r292:

Локальное продолжение 08.09.2026: **r292**, база r291. Исправлены B35–B38:
чтение токенов, отказ hash-проверки, различение 503/401 и установочный guard.
[Реестр](bug-register.ru.md) — 38 карточек, [отчёт](audit-fixes-r292.ru.md).
По просьбе владельца проверяется только изменённая область и ближайшие соседи;
полный набор и аппаратные/SDK gates не объявлены пройденными. R03/R08 остаются открыты.

Историческая контрольная точка предыдущего прохода:

Локальное продолжение 08.09.2026: **r291** на основе r290. Исправлены четыре группы
ошибок привязки родительского телефона; 55 новых сценариев, 1038 общих тестов проходят.
[Реестр](bug-register.ru.md) — 34 карточки, [подробный отчёт](audit-fixes-r291.ru.md).
Следующие приоритеты: общая согласованность UCI/писателей токенов (R03/R08) и аппаратные
проверки. Exclusive публикация токена не объявляется общим завершением этих задач.
Приоритет D01 и ограничения управления роутером не изменены. Нет новой SDK/CI/device проверки.
...
5. Для исполняемого кода перед push запустить `quality:changed`; полный `quality:gate` нужен только по критериям `test-strategy.ru.md`. Android, GitHub SDK и живой роутер остаются отдельными доказательствами.
