...
Актуализировано: **22 июля 2026 года**.
...
- версия OpenWrt-пакета в ветке — `0.1.0-r249`;
...
Прогресс 21 июля 2026 года: первый пункт начат и доведён до самостоятельной границы. `features/settings/general.js` теперь строит вкладку `Общие`, а `features/settings/persistence.js` проверяет черновик, раскладывает его по `global/usb/cloud/gdrive/adguard` и получает UCI/commit только через узкий адаптер. Единый порядок сохранения и runtime side effects пока намеренно остаётся в координаторе. Пункт 2 выполнен в ветке `chatGPT`: определения вкладок, deep-link и выбранное дочернее состояние вынесены в `features/navigation/state.js`; возврат в раздел сохраняет текущую страницу. Следующий проход — пункт 3, команды и обратная связь (§frontmod).
...
Прогресс 22 июля 2026 года: после transaction/persistence cleanup проход `r249` продолжил уменьшение `overview.js` без изменения runtime-порядка. Общие Settings fields, вкладки Misc/Storage, AI presentation и device-type listbox теперь имеют отдельных владельцев. Следующие извлечения должны оставаться поведенчески нейтральными и сопровождаться browser regression; mobile hit targets и живые router/phone доказательства остаются активными P1-задачами (§frontmod, §settingview1, §uxrev01).
