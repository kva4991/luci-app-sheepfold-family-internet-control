...
Эта инструкция настраивает legacy-канал **«Отзыв / предложения» только для LuCI**. Веб-интерфейс
передаёт сообщение домашнему роутеру, роутер проверяет поля и отправляет JSON в Yandex API
Gateway, а закрытая Cloud Function сохраняет запись в Serverless YDB. Родительское Android-
приложение переведено на [сквозное шифрование и отдельный сервер поддержки](support-report-transport.ru.md)
(§srep001); детское APK отчёты не отправляет.
...
- стандартный и AI-вариант LuCI используют один legacy-канал; Android APK его не вызывает.
