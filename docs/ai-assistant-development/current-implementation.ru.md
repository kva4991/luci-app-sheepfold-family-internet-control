...
- принимает form-urlencoded только через общий строгий декодер, а весь динамический JSON для провайдера и ошибок строит штатным OpenWrt `jshn`; повреждённые `%XX` и NUL отклоняются до обращения к провайдеру (§jsonio1).

Это сделано не ради единого стиля: пользовательский текст находится на границе недоверенного ввода, а ручная строковая сборка JSON легко расходится между провайдерами и пропускает управляющие символы. Один строгий decoder устраняет две независимые копии неоднозначного `%XX`-парсинга, а `jshn` отделяет данные от JSON-синтаксиса. Полный разбор причин и принятых ограничений находится в [ADR-0013](../architecture/decisions/0013-strict-ai-json-boundary.ru.md).
