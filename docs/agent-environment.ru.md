...
Перед Gradle проверить процессную переменную `$env:GRADLE_USER_HOME`: она может быть
унаследована от другого проекта. Ошибка доступа к `gradle-*.zip.lck` в чужом каталоге
не означает повреждение Sheepfold и не требует удаления lock, изменения ACL или
повторной установки Gradle. Указать для текущего процесса уже подготовленный кеш
Sheepfold/пользователя и повторить разрешённый wrapper-запуск; глобальную переменную
Windows и чужой проект не менять. На этом компьютере проверенный пользовательский
кеш: `C:\Users\User\.gradle`. 31.08.2026 агрегатный `npm.cmd run lint` в песочнице
упёрся в такой унаследованный путь после успешного ESLint; самостоятельные parent/child
wrapper с правильным окружением прошли. Это не означает успех прерванного агрегата.
