...
## Миграция фиксированного порядка r287

Решение владельца от 07.09.2026: `no_restrictions blocklist admin_devices allowlist global_block temp_access device_schedule group_schedule default_access`.
`postinst` заменяет только два известных прежних стандартных значения `access_priority`.
Тестовый IPK-сборщик берёт этот фрагмент миграции и права общих библиотек прямо из Makefile.
Неизвестные пользовательские строки сохраняются, но не интерпретируются как работающий
редактируемый порядок: UI остаётся read-only, runtime использует фиксированный общий расчёт.
Членство в списках, группы и карантин эта миграция не изменяет.
