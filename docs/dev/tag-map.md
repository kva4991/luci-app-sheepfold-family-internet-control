...
| `§frontmod` | FRONTEND_BY_DOMAIN | архитектура | LuCI frontend делится на четырёхстрочную route-точку входа в `view/sheepfold`, явную композицию в `features/overview/application.js`, инфраструктуру в `sheepfold/core`, общие UI-части в `sheepfold/shared` и предметные модули в `sheepfold/features/<area>`. Не создавать общий склад `utils.js`; новые извлечения делать по самостоятельному контракту, а не ради счётчика строк. Файлы: `CODING_RULES.md`, `AGENTS.md`, `docs/frontend-architecture.ru.md` |
| `§iconcat1` | PROJECT_ICON_CATALOG | LuCI/Android/UX | Собственные значки имеют уникальные английские `camelCase`-имена и SVG-геометрию только в `icons/catalog.json`. LuCI-реестр, HTML-каталог и Android Vector Drawable генерируются; inline-SVG и символы действий в экранах запрещены. Файлы: `docs/icon-catalog.ru.md`, `icons/`, `generateIconCatalog.mjs`, `shared/icon-registry.js`, `iconCatalog.test.mjs`, ADR-0017 |
| `§utils1` | NON_CANONICAL_UTILITIES | структура/агенты | `utilities/` хранит полезные сопутствующие материалы с объяснённой причиной сохранения, но не продуктовые контракты, обязательные инструменты, секреты или build-артефакты. Принятое решение переносится в `docs/`/ADR, рабочий helper — в `scripts/` или `tools/`. Файлы: `utilities/README.ru.md`, `AGENTS.md`, `docs/developer-task.ru.md` |
...
| `§homen01` | TRUSTED_HOME_NETWORK_ENDPOINTS | сеть/API/security | Дополнительный Sheepfold endpoint разрешается только в явно доверенном локальном сегменте после изоляции API в отдельный web root. Публичный WAN, CGNAT, UPnP и безусловное правило `WAN -> 5201` запрещены. Discovery сообщает минимальный список кандидатов без секретов; доверие подтверждает SPKI. NAT-сегмент без криптографической личности клиента отклоняется. Файлы: `home-network-access.ru.md`, `0016-trusted-home-network-access.ru.md`, `android-openwrt-api.ru.md`, `security.ru.md` |
| `§pairux1` | SIMPLE_ONE_STEP_ADMIN_PAIRING | pairing/UX/security | QR-сопряжение намеренно не требует второго кода в Android и LuCI. Первый успешный телефон получает credential, backend атомарно сжигает короткоживущий код, а LuCI предупреждает не показывать и не пересылать QR. Одноразовость не скрывает остаточный риск кражи первого использования. Файлы: `home-network-access.ru.md`, `0016-trusted-home-network-access.ru.md`, `security.ru.md`, `features/administrators/editor.js`, `administratorEditorUi.test.mjs` |
...
| `§country1` | COUNTRY_PROFILE_EMERGENCY_SITES | Выбранный при установке или в LuCI профиль `ru/by/cn/other` управляет только собственными аварийными карточками. Нейтральный `other` не добавляет заводских сайтов. Ручная правка снимает профильное владение, удаление создаёт постоянное исключение, широкие порталы/поиск/маркетплейсы/такси не входят в defaults. Файлы: `country-profiles*.md`, `country-profiles/*.json`, `sheepfold-country-profile`, `features/emergency/sites.js`, `countryProfiles.test.mjs`. |
...
| `§dscqr01` | ANDROID_DISCOVERY_AND_PAIRING_ERRORS | Android/API | Оба APK читают well-known через штатный HTTPS, узнают фактический API-порт и проверяют отдельный `/ping`; детский APK ищет шлюз до 30 секунд перед ручным IP. QR виден только после backend-активации, новый код сбрасывает старый лимит, а DHCP/neighbour-ожидание не считается неверным кодом. Android не отправляет постоянно видимый QR по кругу после ошибки, принимает новый payload на том же экране и разрешает намеренный повтор прежнего только после пустых кадров. LuCI бледнит QR и показывает «QR-код использован» только после backend-статуса `paired=1`, а не по факту распознавания камерой. Form-urlencoded разбирается без pipeline-зависимого цикла BusyBox. Ошибки не бывают пустыми/`undefined`; после сопряжения родительский APK восстанавливает порт только через закреплённый сертификат и не повторяет неоднозначную команду. Debug APK пишет маскированную трассировку в Downloads, а opt-in router trace в RAM показывает этап backend без QR, кода, токена и MAC. Backend ищет MAC в DHCP, neighbour и ARP. Файлы: `DiagnosticLog.kt`, `LocalRouterDiscovery.kt`, `ChildRouterDiscovery.kt`, `RouterEndpointRecovery.kt`, `SecureRouterConnectionManager.kt`, `SafeRouterSetupScreen.kt`, `features/administrators/controller.js`, `features/administrators/editor.js`, `sheepfold-pair-diagnostics`, `sheepfold-api-pair`, `sheepfold-pair-common`, `android-config.ru.md`, `troubleshooting.ru.md` |
...
| `§authrs1` | ANDROID_ROUTER_SESSION_RECOVERY | Android/API/security | `401`, явный окончательный token-код, отвязка админского устройства и несовпадение HTTPS/SPKI требуют новой явной привязки. Очищаются только endpoint/token/device/TLS credential; соглашение, Android-разрешения и защита приложения сохраняются. Тайм-ауты, offline и `5xx` токен не удаляют. Для multi-IP hostname сначала выполняются безопасные `/ping`-пробы, после чего одноразовый код отправляется ровно одному endpoint и не повторяется после неоднозначного сбоя. Сессия читается один раз, а `RouterConnectionRequest` сохраняет identity equality: структурно равный `data class` несовместим со слабой картой секретов и может потерять TLS pin после GC. Файлы: `RouterSessionRecovery.kt`, `RouterConnectionRequest.kt`, `PairingSessionToken.kt`, `SheepfoldConnectionStore.kt`, `RouterAdminClient.kt`, `AiAssistantClient.kt`, `SecureRouterConnectionManager.kt`, `MainActivity.kt`, `SafeRouterSetupScreen.kt`, `sheepfold-api`, `sheepfold-api-legacy`, `androidRouterSessionRecovery.test.mjs`, `android-config.ru.md` |
| `§uicontrast` | ANDROID_UI_CONTRAST | Android/UX/tests | Светлая и тёмная Material-палитры сохраняют не менее `4.5:1` для обычного текста и не менее `3:1` для акцентных, крупных и намеренно бледных управляющих элементов. Alpha-композиция считается по фактическому фону; наличие двух разных hex-строк само по себе контраст не доказывает. Статический тест не заменяет снимок физического устройства. Файлы: `Theme.kt`, `ControlMenuTabs.kt`, `androidUiContrast.test.mjs`, `test-strategy.ru.md`, `testing-cases.ru.md` |
...
| `§meshidea1` | OPTIONAL_MESH_AND_ROAMING_WIZARD | продуктовая идея | Необязательный мастер для нескольких OpenWRT-роутеров: проводной backhaul предпочтителен, беспроводной строится поверх проверенного `mesh11sd`/`802.11s`, роуминг использует `802.11k/v/r` и `usteer`, а Huawei Mesh+ остаётся функцией штатной прошивки. Файл: `development-ideas.md` |
...
| `§jsonio1` | STRICT_AI_JSON_BOUNDARY | AI/backend/security | Динамический JSON AI-gate/handler строится через OpenWrt `jshn`; общий `sheepfold-lib-form` строго разбирает form-urlencoded для Android admin API и AI, а AI-only `sheepfold-lib-json` владеет provider payload. Локальные escape/decode-копии, `eval`, `printf %b` и ручная сборка provider payload запрещены. Read-only live-router тест обратно разбирает chat/Gemini payload штатным `jshn` без обращения к провайдеру. Файлы: `0013-strict-ai-json-boundary.ru.md`, `sheepfold-lib-form`, `sheepfold-lib-json`, `sheepfold-api-admin-config`, `sheepfold-ai-gate`, `sheepfold-ai-handler`, `scripts/sheepfold_variants.py`, `aiJsonBoundary.test.mjs`, `adminConfigApi.test.mjs`, `remoteChecks.sh` |
| `§andwork1` | ANDROID_BACKGROUND_WORK_SPLIT | Android/фоновые задачи | Периодическая и загрузочная сетевая проверка детского APK выполняется уникальными WorkManager-задачами с network constraint и интервалами 15/30 минут. AlarmManager остаётся только для известного времени окончания доступа; receiver не запускает сеть и coroutine. Файлы: `0012-android-background-work.ru.md`, `PollingScheduler.kt`, `StatusPollWorker.kt`, `SafeBootReceiver.kt`, `androidChildPolling.test.mjs` |
| `§auditopt1` | DEFERRED_AUDIT_OPTIMIZATIONS | архитектура/идеи | Транспорт Android, пакетное чтение UCI и индекс расписаний меняются только после измерения реальной проблемы и проверки границ безопасности. Нативный `uci.apply()` не заменяет Sheepfold persistence adapter, потому что не владеет staging, optimistic revision и runtime rollback. Файлы: `development-ideas.md`, `CODING_RULES.md` |
...
| `§bbxsort1` | BUSYBOX_SORT_ATOMIC_REWRITE | shell/OpenWrt/совместимость | BusyBox `sort` целевого OpenWrt не поддерживает GNU `-o`; сортировка на месте выполняется через соседний временный файл и атомарный `mv`, иначе stdout загрязняет машинный ответ, а исходный файл остаётся с дублями. Файлы: `CODING_RULES.md`, `agent-gotchas.ru.md`, `sheepfold-emergency-sites`, `emergencySites.test.mjs`, `portableShellAndStatus.test.mjs` |
| `§awkport1` | AWK_INTERVAL_PORTABILITY | shell/OpenWrt/CI | Интервалы regexp `{n,m}` не используются внутри `awk`, потому что Ubuntu `mawk` и BusyBox `awk` могут обработать их по-разному. Строгий regexp выполняет `grep -E`, длину проверяют через `length()`, а тест защищает обе границы. Файлы: `CODING_RULES.md`, `agent-gotchas.ru.md`, `sheepfold-adguard`, `sheepfold-sim-monitor`, `portableShellAndStatus.test.mjs` |
| `§actsub1` | COALESCED_ACTION_SUBSCRIBERS | LuCI/API/состояние | Одинаковый стабильный action-key создаёт одну backend-мутацию, но каждый UI-инициатор сохраняет собственные success/error/refresh callbacks. Элементы управления разблокируются после операции и всех подписчиков. Файлы: `0014-coalesced-luci-action-subscribers.ru.md`, `frontend-architecture.ru.md`, `core/backend/actions.js`, `luciCommandActions.test.mjs` |
...
| `§aisrvexp1` | AI_SERVER_CORE_EXPERIMENT | AI/эксперимент | Изолированный серверный каркас проверяет модульный pipeline, строгие контракты, safety-ворота и eval-сценарии, но не входит в IPK/APK/LuCI и не получает реальные семейные данные. Канонические продуктовые правила остаются в `docs/ai-assistant-development/`; перенос в runtime требует отдельного ADR и профильной проверки. Файлы: `experimental/README.ru.md`, `experimental/ai-server-core/`, `0020-isolated-ai-server-core-experiment.ru.md` |
...
| `§aialert1` | AI_TRUSTED_SAFETY_ADVISORIES | AI/уведомления | Комендантский час проверяется по подтверждённому региону, возрасту, дате и официальному источнику, а предупреждения о мошенничестве поступают только из подписанной версионированной ленты; оба вида сообщений информируют, но не меняют правила роутера и не исполняют удалённое содержимое. Файлы: `ai-assistant-development/trusted-safety-advisories.ru.md`, `README.md`, `modules.ru.md`, `implementation-roadmap.ru.md` |
...
| `§secaudit1` | MANUAL_AGENTIC_SECURITY_AUDIT | безопасность/тестирование/LLM | Внешний VVAH запускается только вручную на отдельном чистом clone: по умолчанию только doctor/estimate, передача исходников требует двух явных флагов, scan останавливается после S9 и не исправляет код. Находка является кандидатом до root-cause проверки и детерминированного regression test. Файлы: `manual-agentic-security-audit.ru.md`, ADR-0015, `tools/security/runVvaharnessAudit.ps1`, `agenticSecurityAudit.test.mjs` |
...
| `§slstres` | SITE_LIST_RESILIENCE | списки сайтов/backend | Каждый внешний источник ограниченно проверяется отдельно, рабочий кэш заменяется атомарно, полностью плохой или подозрительно укороченный источник сохраняет последнюю копию, повторяется через сутки и после трёх циклов уведомляет администраторов; после восстановления отправляется одно событие. Внешний transport только HTTPS, HTTP разрешён лишь для `127.0.0.1`. Скачивание не считается доказательством применения DNS/firewall. Файлы: `architecture/decisions/0019-site-list-ingestion-safety.ru.md`, `site-list-sources.ru.md`, `sheepfold-site-lists`, `siteListResilience.test.mjs` |
...
| `§autoact1` | ACTION_AUTOMATION_PROFILE | настройки/устройства/UX | `automation_mode=maximum` скрывает дочерние поля и атомарно задаёт разрешение новых устройств, полное автоопределение, автоматические группы и автоматический identity-карантин; `selective` показывает и сохраняет отдельные выборы. Миграция никогда не объявляет изменённый старый конфиг максимальным. Файлы: ADR-0018, `product-requirements.md`, `device-passport-and-control.ru.md`, `features/settings/general.js`, settings persistence, UCI defaults/postinst, `install.sh`, `frontendDomainModels.test.mjs` |
...
| `§apicon1` | ROUTER_API_CONTRACT | API/совместимость | Router backend владеет решением и подтверждает persistence/runtime; клиенты получают структурированные status/errorCode, безопасные defaults и capability/version evolution. Неоднозначные неидемпотентные команды не повторяются автоматически. Родительский admin-config имеет одну точку диспетчеризации, отдельную read-model и небольшие доменные модули, загруженные в общий транзакционный процесс; Android так же отделяет transport/commands от JSON decoding и моделей. Файлы: `api-contracts.ru.md`, `backend-design.ru.md`, `android-openwrt-api.ru.md`, `sheepfold-api-admin-config`, `sheepfold-lib-admin-config-*`, `RouterAdminClient.kt`, `RouterAdminJson.kt`, `RouterAdminModels.kt` |
...
| `§grpsch1` | CANONICAL_GROUP_SCHEDULE_LINKS | расписания/группы/UCI/API | Единственная постоянная связь группы с расписанием хранится как ID секции группы в `schedule.targets`; `group.schedules` только мигрируется, Android явно объявляет поддержку записи, а переименование не ломает правило. Файлы: ADR-0021, `schedules.ru.md`, group/schedule persistence, admin-config API, Android group editor, `auditRuntimeCorrections.test.mjs` |
