...
| `§jsonio1` | STRICT_AI_JSON_BOUNDARY | AI/backend/security | Динамический JSON AI-gate/handler строится через OpenWrt `jshn`, а form-urlencoded разбирается общей AI-only библиотекой со строгой проверкой `%XX` и запретом NUL. Локальные escape/decode-копии, `eval`, `printf %b` и ручная сборка provider payload запрещены. Read-only live-router тест обратно разбирает chat/Gemini payload штатным `jshn` без обращения к провайдеру. Файлы: `0013-strict-ai-json-boundary.ru.md`, `sheepfold-lib-json`, `sheepfold-ai-gate`, `sheepfold-ai-handler`, `scripts/sheepfold_variants.py`, `aiJsonBoundary.test.mjs`, `remoteChecks.sh` |
