...
| `§routerharness` | LIVE_ROUTER_AUTOMATION | тестирование/OpenWrt/LuCI | Исполняемый безопасный контур живого тестового роутера: отдельный SSH-ключ, частный IPv4, DPAPI для LuCI, backup до изменений, SHA-256 пакета, фиктивный MAC с обязательным restore и read-only Playwright desktop/mobile. Hardware-команды, reboot и массовое обновление пакетов запрещены. Файлы: `live-router-automation.ru.md`, `live-router-testing.ru.md`, `tools/router-testing/`, `liveRouterHarness.test.mjs`, `AGENTS.md` |
