...
| `§pm3kq7r` | NAMING_BY_LAYER | стиль | Именование зависит от слоя: JS/Kotlin/JSON — camelCase; OpenWrt UCI, shell и Makefile — нативный стиль OpenWrt. Новые локальные переменные/приватные поля имеют говорящее имя до 15 символов без непонятных сокращений. Переименование UCI-полей — только с миграцией. Файлы: `CODING_RULES.md`, `AGENTS.md`, `ai-dev-guide.ru.md` |
...
| `§dscqr01` | ANDROID_DISCOVERY_AND_PAIRING_ERRORS | Android/API | Автопоиск читает well-known через штатный HTTPS, узнаёт фактический API-порт, проверяет endpoint и никогда не показывает пустую/`undefined` ошибку QR-сопряжения; backend ищет MAC телефона в DHCP, neighbour и ARP. Файлы: `LocalRouterDiscovery.kt`, `SecureRouterConnectionManager.kt`, `sheepfold-pair-common`, `android-config.ru.md`, `troubleshooting.ru.md` |
| `§prodvar` | STANDARD_AND_AI_PRODUCT_VARIANTS | архитектура/релизы | `Sheepfold` и `Sheepfold - AI Support` собираются из одного общего ядра; Standard физически не содержит AI/activity-модули, а общий контроль не дублируется между репозиториями. Файлы: `product-variants.ru.md`, `product-variants-implementation-task.ru.md`, `AGENTS.md`, `developer-task.ru.md` |
