...
| Если изменяется | Обязательно актуализировать или проверить | Минимальное доказательство | Почему |
|---|---|---|---|
| Новый или существенно изменённый test | purpose header, `tests/categories.mjs`, direct command, ожидаемый результат, limits и common failures в профильном runbook | direct test + `testCategories.test.mjs` | Зелёный тест без понятной границы создаёт ложную уверенность |
