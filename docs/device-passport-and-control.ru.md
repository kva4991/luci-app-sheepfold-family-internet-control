...
| Серый перечёркнутый значок человека с шестерёнкой | карточка пока защищена в основном MAC; надёжно заметить подмену нельзя | устройство вредоносное, заблокированное или неисправное |
| Красный перечёркнутый значок человека с шестерёнкой | текущие устойчивые признаки не совпали с доверенным baseline и подключение помещено в identity-карантин | окончательное доказательство умышленной подмены |
...
Если одновременно показаны, например, корона и серый перечёркнутый identity-значок, это не
...
Первые три пользовательские настройки собраны в раздел `Автоматизация действий` (§autoact1). В заводском режиме `Максимальная автоматизация` они скрыты и получают согласованный профиль `allow + full + automatic`. Режим `Выборочная автоматизация` показывает их и сохраняет ручной выбор родителя. Обновление старого конфига включает максимальный режим только при точном совпадении всех прежних значений с автоматическим профилем.
...
Фиксированный порядок интернет-политики устройства (решение владельца от 07.09.2026). Членство в чёрном списке независимо закрывает управление роутером, даже когда группа разрешает интернет:

```text
identity quarantine (отдельный защитный слой)
→ группа «Без ограничений»
→ постоянный чёрный список устройств
...
6. Если у прежней карточки был только серый MAC-only индикатор, надёжно обнаружить подмену
   невозможно.
