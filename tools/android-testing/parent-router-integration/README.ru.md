...
Для смены LAN / основной домашний Wi-Fi использовать отдельный ручной
`HomeNetworkLiveTest` по [runbook домашнего доступа](../../../docs/home-network-access.ru.md#проверки-v1).
Обычные девять paired-read checks не подтверждают этот маршрут. Допуск на роутере должен
быть включён явно; при `device_source_mismatch` сохранять привязку и проверять MAC сетей,
не удалять токен и не открывать generic WAN-доступ ради прохождения теста.
