...
Он сначала спрашивает язык приложения (`ru` по умолчанию, `en` для английского), затем вариант Sheepfold, согласие с пользовательским соглашением и режим автонастройки. По умолчанию включается полный режим, в котором Sheepfold сможет автоматически назначать уверенно распознанные инфраструктурные устройства в группу `Без ограничений`. Урезанный режим можно выбрать явно для роутеров с очень малым запасом места: он избегает тяжёлых проверок портов, но тоже может автоназначать уверенно распознанные инфраструктурные устройства.
...
Репозиторий находится в активной разработке экспериментальной версии. LuCI использует единый защищённый runner для изменяющих router-команд; одинаковые нажатия объединяются, а глобальный internet toggle подтверждается backend-ом. UCI, device/DHCP/firewall, Wi-Fi reload, backup rollback и pairing разделены на узкие persistence-адаптеры. Финальная зачистка удаляет дублирующие forwarding-функции из coordinator, не меняя порядок runtime-применения. Расписания, группы, settings side effects и discovery payload также вынесены из LuCI-координатора. LuCI/backend, nftables-правила, расписания, защищённое Android-сопряжение и базовый Telegram-адаптер уже реализованы; до stable остаются полная живая матрица OpenWrt/интеграций, физические Android-проверки, production signing и ограничения из roadmap.
...
LuCI `r249` дополнительно выносит из `overview.js` общие поля настроек, полную композицию Misc/Storage, AI presentation и выбор типа устройства; coordinator остаётся точкой сборки, а не владельцем этих DOM-деревьев.

LuCI `r250` завершает декомпозицию `overview.js`: публичный файл — четырёхстрочный bootstrap, реальная сборка находится в `features/overview/application.js`, а предметная логика распределена по контроллерам.

LuCI `r252` — повторно проверенная финализация декомпозиции `overview.js`: четырёхстрочный bootstrap, строгий require/dependency/UCI audit и честные partial-persistence/refresh результаты. Аппаратная firewall/DNS/Wi-Fi матрица остаётся обязательной перед релизом. §ovaudit5
