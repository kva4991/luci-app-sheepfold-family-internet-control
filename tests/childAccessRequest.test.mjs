...
const buildScript = read('scripts/build-test-ipk.py');
...
describe('Optional child request for 30 minutes', () => {
  it('is disabled per administrator by default and is visible in LuCI settings', () => {
    ...
    assert.match(overview, /never grants internet automatically/);
  });
  ...
  it('ships and validates the router helper in both package variants', () => {
    assert.match(hardening, /ACCESS_REQUEST=.*sheepfold-access-request/);
    assert.match(hardening, /"\$STATUS_API" "\$ACCESS_REQUEST"/);
    assert.match(buildScript, /\/usr\/libexec\/sheepfold\/sheepfold-access-request/);
  });
});
