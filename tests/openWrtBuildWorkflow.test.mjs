...
const workflowPath = join(workflowDirectory, 'build-openwrt-packages.yml');
...
const androidWorkflow = readFileSync(join(workflowDirectory, 'validate-android.yml'), 'utf8');
...
describe('OpenWrt GitHub Actions build §owrtci1', () => {
  ...
  it('uses Node 24 compatible GitHub Actions in every workflow', () => {
    ...
    assert.match(androidWorkflow, /gradle\/actions\/setup-gradle@v6/);
    assert.doesNotMatch(
      allWorkflows,
