...
- Use `npm.cmd run quality:plan` before choosing checks, `quality:changed` for fast local feedback, and `quality:gate` before pushing executable changes (§qassist). Read `docs/quality-assistants/README.ru.md` before changing the map or runner. Never hide an unknown path, `partial` result, or `pendingManualChecks`; classify or report it.
