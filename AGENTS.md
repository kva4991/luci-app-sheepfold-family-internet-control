...
- The family message relay is local-first, parent-APK-only and disabled by default. Its strict protocol, synthetic-only server pilot behind DNS/TLS/Caddy and an unfinished unwired Android client foundation exist. The Android foundation has known handoff gaps, no production provisioning/UI/lifecycle wiring and no completed API 28 device or field gate; the OpenWrt runtime is absent pending a separately approved native crypto helper and target/live-router gates. Keep `clientsReady=no`, `realDataAllowed=no`; no enrollment or real credentials/data have been used. It opens no inbound home port, never proxies LuCI/SSH/the full router API, never receives the local Android Bearer, and keeps working local Wi-Fi independent of server availability.
